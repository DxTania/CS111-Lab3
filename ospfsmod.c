...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
  ...
  while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
    ...
    uint32_t cur_file_byte_offset = (f_pos - 2) * OSPFS_DIRENTRY_SIZE;
    uint32_t file_type;
    if (cur_file_byte_offset >= dir_oi->oi_size) {
      r = 1;
      break;
    }
    ...
    /* EXERCISE: Your code here */
     od = ospfs_inode_data(dir_oi, cur_file_byte_offset);
     entry_oi = ospfs_inode(od->od_ino);
     if (entry_oi == NULL || od->od_ino == 0) {
      // Ignore blank directory entry
      f_pos++;
      continue;
     }
     ...
     ok_so_far = filldir(dirent, od->od_name,
      strlen(od->od_name), f_pos, od->od_ino, file_type);
     f_pos++;
  }
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
  ...
  // Clean up if link count reaches 0
...
{
  // Your code here.
  if (b >= 0 && b < OSPFS_NDIRECT) // direct block
    return b;
  else if (b >= OSPFS_NDIRECT && b < (OSPFS_NDIRECT + OSPFS_NINDIRECT)) // 1st indrect
    return b - OSPFS_NDIRECT;
  else if (b >= OSPFS_NDIRECT + OSPFS_NINDIRECT) // 2nd indirect
    return ((b - (OSPFS_NDIRECT + OSPFS_NINDIRECT)) % OSPFS_NINDIRECT);
  else
    return -1;
}
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
  ...
  while (amount < count && retval >= 0) 
  {
    ...
    if(n > bytes_left_to_write) n = bytes_left_to_write;
    if(copy_from_user(data + data_offset,buffer,n) != 0) return -EFAULT;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
  ...
  /* EXERCISE: Your code here. */
  int off; // Search for an empty entry
  for (off = 0; off < dir_oi->oi_size; off += OSPFS_DIRENTRY_SIZE) {
    ospfs_direntry_t *od = ospfs_inode_data(dir_oi, off);
    if (!od->od_ino)
      return od;
  ...
  // Couldn't find an empty entry, add block (idk if this is right, change size?)
  if (add_block(dir_oi) < 0)
    return ERR_PTR(-ENOSPC);

  // return starting position of new block,
  return (ospfs_direntry_t*) ospfs_inode_data(dir_oi, OSPFS_BLKSIZE *
    (ospfs_size2nblocks(dir_oi->oi_size) - 1));
}
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
  ...
  // check eexist
  if (find_direntry(ospfs_inode(dir->i_ino), dentry->d_name.name,
    dentry->d_name.len)) {
    return -EEXIST;
  }
  ...
  memset(inode, 0, sizeof(ospfs_inode_t));
  inode->oi_mode = mode;
  inode->oi_size = 0;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
  ...
  ospfs_symlink_inode_t *new_inode_loc = (ospfs_symlink_inode_t*) ospfs_inode(entry_ino);
  if (new_inode_loc == NULL) {
    return -EIO;
  }

  // Create the direntry & initialize
  ospfs_direntry_t *od = create_blank_direntry(dir_oi);
  if (IS_ERR(od))
    return PTR_ERR(od);
  ...
  if (qmark && colon && colon > qmark) {
    size_t rootpathlen = colon - qmark;
    size_t otherpathlen = strlen(colon);

    if (rootpathlen + otherpathlen > OSPFS_MAXSYMLINKLEN) {
      return -ENAMETOOLONG;
    }

    new_inode_loc->oi_size = strlen(qmark) + 1;
    strncpy(new_inode_loc->oi_symlink, qmark, rootpathlen);
    new_inode_loc->oi_symlink[rootpathlen] = '\0';
    strncpy(new_inode_loc->oi_symlink+rootpathlen+1, colon, otherpathlen);
    new_inode_loc->oi_symlink[new_inode_loc->oi_size] = '\0';
  } else {
    size_t namelen = strlen(symname);
    if (namelen > OSPFS_MAXSYMLINKLEN) {
      return -ENAMETOOLONG;
    }
    new_inode_loc->oi_size = namelen;
    strncpy(new_inode_loc->oi_symlink, symname, namelen);
    new_inode_loc->oi_symlink[namelen] = '\0';
  }
